...
2. **Handle staging based on current state:**
   - If there are staged changes: Use those for the commit
   - If there are NO staged changes: Stage all unstaged changes with `git add -A`
...
After executing the process, provide a summary in the chat including:
1. Number of files changed and types of changes made
...
4. Any other contextually relevant information about the commit

**Important:** This prompt ALWAYS commits changes. It will stage all unstaged changes if nothing is currently staged, then create and execute the commit.
